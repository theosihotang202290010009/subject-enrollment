package com.theo.enrollment.constant;

public class ResponseMessage {
    public static final String SUCCESSFULLY_CREATE_NEW_DATA ="Successfully create new data.";
    public static final String SUCCESSFULLY_GET_DATA = "Successfully get data";
    public static final String SUCCESSFULLY_GET_ALL_DATA = "Successfully get All data";
    public static final String SUCCESSFULLY_UPDATE_DATA = "Successfully update data";
    public static final String ERROR_NOT_FOUND = "Data not found";
    public static final String SUCCESSFULLY_UPDATE_STATUS = "Successfully update status";
}
